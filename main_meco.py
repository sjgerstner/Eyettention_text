	...
	for train_idx, test_idx in kf.split(split_list):
		...
		for episode_i in range(episode, cf["n_epochs"]+1):
			...
			if np.mean(val_loss) < old_score:
				...
				torch.save(dnn.state_dict(), '{}/CELoss_SAT_{}_eyettention_{}_newloss_fold{}.pth'.format(args.save_data_folder, args.test_mode, args.atten_type, fold_indx))
